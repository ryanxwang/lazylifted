...
pub struct PartialSpaceWeightedRankingConfig {
    pub successor_generator: SuccessorGeneratorName,
    pub graph_compiler: PartialActionCompilerName,
    pub group_partial_actions: bool,
    pub predecessor_weight: f64,
    pub sibling_weight: f64,
}

#[derive(Debug)]
pub struct PartialSpaceWeightedRanking {
    config: PartialSpaceWeightedRankingConfig,
}

impl PartialSpaceWeightedRanking {
    pub fn new(config: &PartialSpaceWeightedRankingConfig) -> Self {
        PartialSpaceWeightedRanking {
            config: config.clone(),
        }
    }
}
...
impl DataGenerator for PartialSpaceWeightedRanking {
    fn generate(&self, training_instances: &[TrainingInstance]) -> TrainingData<Vec<CGraph>> {
        ...
        let mut predecessor_pairs_count = 0;
        let mut sibling_pairs_count = 0;
                ...
                for partial_depth in 0..=(chosen_action.instantiation.len()) {
                    ...
                    if let Some(predecessor_index) = &predecessor_index {
                        pairs.push(RankingPair {
                            ...
                            importance: self.config.predecessor_weight,
                        });
        ...
        info!(
            predecessor_pairs_count = predecessor_pairs_count,
            sibling_pairs_count = sibling_pairs_count,
        );
